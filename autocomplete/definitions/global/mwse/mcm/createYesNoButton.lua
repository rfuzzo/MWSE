return {
	type = "function",
	description = [[Creates a new YesNoButton inside given `parent` menu.
...
The same is done by this function if you pass both `parent` and `data` arguments.
	...
	arguments = {
		{ name = "parent", type = "tes3uiElement|mwse.mcm.createYesNoButton.data", description = "The UI element inside which the new YesNoButton will be created." },
		{
			name = "data",
			type = "table",
			optional = true,
			tableParams = {
				...
				{ name = "variable", type = "mwseMCMVariable|mwseMCMSettingNewVariable", description = "A variable for this Button." },
				...
				{ name = "postCreate", type = "fun(self: mwseMCMYesNoButton)", optional = true, description = "Can define a custom formatting function to make adjustments to any element saved in `self.elements`." },
			}
		}
	},
	returns = {{
		name = "button", type = "mwseMCMYesNoButton"
	}}
}
