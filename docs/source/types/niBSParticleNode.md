# niBSParticleNode
<div class="search_terms" style="display: none">nibsparticlenode, bsparticlenode</div>
...
### `followMode`
<div class="search_terms" style="display: none">followmode</div>

Controls if the node and its children follow the parent, or is they are anchored in world space at their initial position. This flag must be set before the first update.
...
```lua
local node = niBSParticleNode.new()
```

**Returns**:

* `node` ([niBSParticleNode](../types/niBSParticleNode.md))
