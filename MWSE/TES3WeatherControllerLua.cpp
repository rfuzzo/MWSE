...
namespace mwse::lua {
	void bindTES3WeatherController() {
		...
		{
			using WeatherParticle = TES3::WeatherController::Particle;

			// Start our usertype.
			auto usertypeDefinition = state.new_usertype<WeatherParticle>("tes3weatherControllerParticle");
		...
		// Bind TES3::WeatherController
		{
			...
			usertypeDefinition["snowFallSpeedScale"] = &TES3::WeatherController::snowFallSpeedScale;
